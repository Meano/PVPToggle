...
public class PvPLocalisation {
	...
	private static String replaceVars(String input, String[] search, String[] replace) {
		...
		return input.replaceAll("(&([a-f0-9]))", "¡ì$2");
	}
	...
	public static enum Strings {
		...
		PLAYER_COOLDOWN(
				"&cYou were just in combat, can't disable PvP yet!"),
		...
		PLAYER_CHECK_OTHER_STATUS(
				"&6%target%'s PvP Status in world %world%: %status%"),
		PLAYER_CHECK_WORLD_STATUS(
				"&6World-wide PvP Status in %world%: %status%"),
		...
		PVP_PLAYER_OTHER_TOGGLE(
				"&6PvP %status% in world %world% by %sender%!"),
		PVP_PLAYER_GLOBAL_TOGGLE(
				"&6PvP %status% across all worlds by %sender%!"),
		PVP_WORLD_TOGGLE(""),
		PVP_GLOBAL_TOGGLE(""),

		PVP_PLAYER_OTHER_TOGGLE_SENDER(
				"&6Successfully %status% PvP for player %target% in world %world%!"),
		PVP_PLAYER_GLOBAL_TOGGLE_SENDER(
				"&6Successfully %status% PvP for player %target% across all worlds!"),
		PVP_WORLD_TOGGLE_SENDER(
				"&6Successfully %status% world-wide PvP in %world%"),
		PVP_GLOBAL_TOGGLE_SENDER("&6Successfully %status% global PvP!"),

		PVP_RESET_PLAYER(
				"&6PvP status reset to login default in world %world% by %sender%"),
		PVP_RESET_PLAYER_GLOBAL(
				"&6PvP status reset to login default across all worlds by %sender%"),
		PVP_RESET_WORLD("&6PvP %status% in world %world% by %sender%!"),
		PVP_RESET_GLOBAL(
				"&6PvP globally reset across all worlds by %sender%!"),

		PVP_RESET_PLAYER_SENDER(
				"&6Successfully reset PvP status of %target% to default in world %world%!"),
		PVP_RESET_PLAYER_GLOBAL_SENDER(
				"&6Successfully reset PvP status of %target% to default!"),
		PVP_RESET_WORLD_SENDER(
				"&6Successfully reset all players PvP to %status% in world %world%"),
		PVP_RESET_GLOBAL_SENDER(
				"&6Successfully reset all players PvP across all worlds!"),

		WORLDGUARD_REGION_ENTERED(
				"&6Forced PvP %status% region '%target%' entered"),
		WORLDGUARD_REGION_EXIT(
				"&6Forced PvP %status% region '%target%' exited"),
		WORLDGUARD_TOGGLE_DENIED(
				"&cPvP is %status% in %target% region!"),
		WORLDGUARD_REGION_ADDED(
				"&6Region %target% added to world %world%"),
		WORLDGUARD_REGION_REMOVED(
				"&6Region %target% removed from world %world%"),
