...
public class World extends PvPCommand {
	...
	protected boolean processCommand() {
		...
		if (this.args[0].startsWith("w:"))
			worldname = isWorld(this.sender, this.args[0].substring(2));
		else if (this.args.length > 1) {
			worldname = isWorld(this.sender, this.args[1]);
		} else {
		...
		if (worldname != null) {
			if ((action.equalsIgnoreCase("status")) || (worldname.equalsIgnoreCase(action)) || ((worldname.equalsIgnoreCase(action.substring(2))) && (action.startsWith("w:")))) {
				getWorldStatus(this.sender, worldname);
			} else if (action.equalsIgnoreCase("reset")) {
				resetWorld(this.sender, worldname, this.plugin.getWorldDefault(worldname));
			} else if ((action.equalsIgnoreCase("on")) || (action.equalsIgnoreCase("off"))) {
				toggleWorld(this.sender, worldname, checkNewValue(action).booleanValue());
			} else {
				sendUsage(this.sender);
			}
		}
