		...
		if (this.args.length > 2) {
			if ((this.args[1].equalsIgnoreCase("add")) || (this.args[1].equalsIgnoreCase("a"))) {
				if (this.args.length > 3) {
					String world = isWorld(this.sender, this.args[3]);
					if (world != null)
						addRegion(this.sender, world, this.args[2]);
				} else if ((this.sender instanceof Player)) {
					addRegion(this.sender, this.player.getWorld().getName(), this.args[2]);
				} else {
					sendUsage(this.sender);
				}
			} else if ((this.args[1].equalsIgnoreCase("remove")) || (this.args[1].equalsIgnoreCase("r"))) {
				if (this.args.length > 3) {
					String world = isWorld(this.sender, this.args[3]);
					if (world != null)
						removeRegion(this.sender, world, this.args[2]);
				} else if ((this.sender instanceof Player)) {
